...
- **Long-running ordinary task driver**: `mcp/tasks/` defines the protocol-neutral `McpTaskDriver` contract and normalized `TaskSnapshot` states (`submitted`, `working`, `input_required`, `completed`, `failed`, `cancelled`). `extensions_config.json -> mcpServers.<server>.task_toolsets` binds exact raw submit/status/cancel names; one raw tool may occupy only one role across that server's groups. `mcp/tools.py` hides status/cancel and replaces submit with a wrapper that returns only the local task ID after persistence. `ordinary.py` reads only MCP `structuredContent`, maps remote `running` to `working`, and treats `error_code=task_not_found` or malformed structured output as permanent failure. `task_tool_caller.py` restores the same `(server_name, user_id:thread_id)` stdio session scope; HTTP/SSE calls remain ephemeral, apply `session_init_timeout` to initialization and `tool_call_timeout` to task calls, and support server-level OAuth refresh outside an Agent run. `persistence/mcp_tasks/` owns the durable remote-handle mapping, poll schedule, immutable notification snapshots/versions, cancellation requests, bounded result/preview/artifact fields, and separate leases for polling, cancellation, and notification dispatch. `app/mcp_tasks/McpTaskService` polls outside the Agent/LLM loop, applies results only under an unexpired lease, fences stale poll results after cancellation, exponentially backs off transient failures, wakes the current thread through an idempotent RunManager key, and marks notification delivery only after that run succeeds. `list_background_tasks` and `cancel_background_task` are always-available framework tools when MCP tasks are enabled; public APIs and tools never expose remote handles. `ThreadState.background_tasks` is a bounded projection refreshed at run start, while SQL remains authoritative. If submission succeeds but persistence fails, the service best-effort cancels the remote task. Durable recovery requires SQL (`sqlite` or `postgres`); configured task toolsets fail startup when the runtime is disabled or persistence is memory. `input_required` remains notification-only and cannot resume the remote task yet.
...
- `migrations/versions/0013_mcp_task_notifications.py` — adds reliable task notification outbox state, cancellation retry state, and run idempotency keys
